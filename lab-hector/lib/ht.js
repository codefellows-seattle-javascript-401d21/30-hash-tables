...
HashTable.prototype.hashKey = function(key) {
  // returns a value for the index of correlating SLL
...
HashTable.prototype.set = function(key, value) {
  ...
  while (currNode) {
    ...
    if (currNode.val && currNode.val[0] === key)
      return new Error('Set Error: Key already exists.');
...
HashTable.prototype.get = function(key) {
  ...
  if (!this.memory[this.hashKey(key)])
    return new Error('Get Error: Bucket does not exist');
...
HashTable.prototype.remove = function(key) {
  ...
  if (!this.memory[this.hashKey(key)])
    return new Error('Remove Error: Bucket does not exist');
